module.exports = {
  ...
  html(isEvent, data) {
    return `
    <div style="position:absolute;bottom:0px;border: 1px solid #222;background:#000;color:#999;padding:3px;right:0px;z-index:999999">Версия 1.2</div>
...
<tab-system style="margin-top: 20px;">
  ...
  <tab label="Текст" icon="align left">
    <div style="padding: 8px;">
      <textarea id="message" class="dbm_monospace" rows="10" placeholder="Введите сообщение здесь..." style="height: calc(100vh - 309px); white-space: nowrap; resize: none;"></textarea>
    </div>
  </tab>
  ...
  <tab label="Эмбеды" icon="book image">
    <div style="padding: 8px;">
      ...
      <dialog-list id="embeds" fields='["title", "url", "color", "timestamp", "imageUrl", "thumbUrl", "description", "fields", "author", "authorUrl", "authorIcon", "footerText", "footerIconUrl"]' dialogTitle="Embed Info" dialogWidth="540" dialogHeight="460" listLabel="Эмбеды" listStyle="height: calc(100vh - 350px);" itemName="Embed" itemCols="1" itemHeight="30px;" itemTextFunction="data.title + ' - ' + data.description" itemStyle="text-align: left; line-height: 30px;">
            ...
            <tab label="Главное" icon="certificate">
              <div style="padding: 8px">
                <div style="float: left; width: calc(50% - 12px);">
                  ...
                  <span class="dbminputlabel">Цвет (Только HEX)</span><br>
                  <table style="width:100%"><tr><td><input id="color" name="actionxinxyla" class="round" type="text" placeholder="Оставьте это поле пустым по умолчанию..."><td>
                ...
                <div style="float: right; width: calc(50% - 12px);">
                  <span class="dbminputlabel">URL-ссылка</span><br>
                ...
                <span class="dbminputlabel">URL-ссылка на картинку</span><br>
                <input id="imageUrl" class="round" type="text" placeholder="Оставьте пустым, если нет...">

                <br>

                <span class="dbminputlabel">URL-ссылка на миниатюру</span><br>
                <input id="thumbUrl" class="round" type="text" placeholder="Оставьте пустым, если нет...">
              </div>
            </tab>

            <tab label="Описание" icon="file image">
              <div style="padding: 8px">
                <textarea id="description" class="dbm_monospace" rows="10" placeholder="Введите здесь описание..." style="height: calc(100vh - 149px); white-space: nowrap; resize: none;"></textarea>
              </div>
            </tab>
              ...
              <div style="padding: 8px">
                <dialog-list id="fields" fields='["name", "value", "inline"]' dialogTitle="Field Info" dialogWidth="540" dialogHeight="300" listLabel="Fields" listStyle="height: calc(100vh - 190px);" itemName="Field" itemCols="1" itemHeight="30px;" itemTextFunction="data.name + '<br>' + data.value" itemStyle="text-align: left; line-height: 30px;">
                  <div style="padding: 16px;">
                    ...
                    <textarea id="value" class="dbm_monospace" rows="10" placeholder="Введите текст поля здесь..." style="height: calc(100vh - 190px); white-space: nowrap; resize: none;"></textarea>
  ...
  <div style="padding: 16px;text-align:center"id="xin4n">Webhook не поддерживает Кнопки</div>
    <div style="padding: 8px;" id="xin4">
      ...
      <dialog-list id="buttons" fields='["name", "type", "id", "row", "url", "emoji", "disabled", "mode", "time", "actions"]' dialogTitle="Button Info" dialogWidth="600" dialogHeight="700" listLabel="Кнопка" listStyle="height: calc(100vh - 350px);" itemName="Button" itemCols="4" itemHeight="40px;" itemTextFunction="data.name" itemStyle="text-align: center; line-height: 40px;">
        <div style="padding: 16px;">
          <div style="width: calc(50% - 12px); float: left;">
            ...
            <select id="type" class="round">
              <option value="PRIMARY" selected>Основной (Синий)</option>
              ...
              <option value="LINK">Ссылки (Серый)</option>
            </select>
            ...
            <option value="PERSONAL">Один раз, только для пользователя команды</option>
            <option value="PUBLIC">Один раз, любой может использовать</option>
            <option value="MULTIPERSONAL">Многоразового, только для пользователя команды</option>
            <option value="MULTI" selected>Многоразового, любой может использовать</option>
            <option value="PERSISTENT">Настойчивый</option>
            </select>
          </div>
          <div style="width: calc(50% - 12px); float: right;">
            <span class="dbminputlabel">Уникальный ID</span>
            <input id="id" placeholder="Оставьте поле пустым, чтобы генерировать автоматически..." class="round" type="text">

            <br>

            <span class="dbminputlabel">Линия действия (1 - 5)</span>
            <input id="row" placeholder="Оставьте это поле пустым по умолчанию..." class="round" type="text">
            ...
            <span class="dbminputlabel">Время (в миллисекундах)</span>
      ...
      <dialog-list id="selectMenus" fields='["placeholder", "id", "tempVarName", "row", "min", "max", "mode", "time", "options", "actions"]' dialogTitle="Select Menu Info" dialogWidth="800" dialogHeight="700" listLabel="Меню" listStyle="height: calc(100vh - 350px);" itemName="Select Menu" itemCols="1" itemHeight="40px;" itemTextFunction="data.placeholder + '<br>' + data.options" itemStyle="text-align: left; line-height: 40px;">
        <div style="padding: 16px;">
          <div style="width: calc(33% - 16px); float: left; margin-right: 16px;">
            ...
            <span class="dbminputlabel">Минимальное кол-во выбора</span>
          ...
          <div style="width: calc(33% - 16px); float: left; margin-right: 16px;">
            ...
            <span class="dbminputlabel">Максимальное кол-во выбора</span>
          ...
          <div style="width: calc(34% - 8px); height: 300px; float: left; margin-left: 8px;">
            ...
            <dialog-list id="options" fields='["label", "description", "value", "emoji", "default"]' dialogTitle="Select Menu Option Info" dialogWidth="360" dialogHeight="440" listLabel="Options" listStyle="height: 210px;" itemName="Option" itemCols="1" itemHeight="20px;" itemTextFunction="data.label" itemStyle="text-align: left; line-height: 20px;">
              <div style="padding: 16px;">
                ...
                <input id="value" placeholder="Текст передается в переменной temp..." class="round" type="text">
                ...
                <span class="dbminputlabel">По умолчанию выбраны</span><br>
  ...
  <tab label="Файл" icon="file image">
    <div style="padding: 8px;">

      <dialog-list id="attachments" fields='["tipo", "url", "canvasvar", "canvasnome", "compress", "name", "spoiler"]' dialogTitle="Informação do Anexo" dialogWidth="400" dialogHeight="480" listLabel="Файловый менеджер" listStyle="height: calc(100vh - 350px);" itemName="File" itemCols="1" itemHeight="30px;" itemTextFunction="glob.formatItem(data)" itemStyle="text-align: left; line-height: 30px;">
    ...
    if (avalue == 2) {
      ...
      document.getElementById('xinxyla3').style.display = 'none';
          ...
          <option value="0">Локальный/Веб-АДРЕС</option>
          <option value="1">Канвас</option>
          <option value="2">Изображения DBM</option>
        </select>
        <br><div id="xinxyla2">
          <span class="dbminputlabel">Локальный/Веб-АДРЕС</span>
...
<br>
<div id="xinxyla3">
          <span class="dbminputlabel">Степень Сжатия</span><br>
          ...
          <span class="dbminputlabel">Имя вложения</span>
          <input id="name" class="round" type="text" placeholder="Оставьте это поле пустым по умолчанию...">

          <br>

          <div style="text-align: center; padding-top: 4px;">
            <dbm-checkbox id="spoiler" label="Сделать спойлер вложения"></dbm-checkbox>
          </div>
        </div>
      </dialog-list>
    </div>
  ...
  <tab label="Конфиг" icon="cogs">
    ...
    <div id="xincheck">
      <dbm-checkbox style="float: left;" id="reply" label="Ответ (Только со Slash)" checked></dbm-checkbox>

      <dbm-checkbox style="float: right;" id="ephemeral" label="Сделать ответ приватным"></dbm-checkbox>
      ...
      <div style="display: flex; justify-content: space-between;">
        ...
        <dbm-checkbox id="dontSend" label="Не отправлять"></dbm-checkbox>
    ...
    <div>
      ...
      <span class="dbminputlabel">Отправить Webhook</span><br>
    ...
    <input id="webhookname" class="round" type="text" style="width:100%" placeholder="По желанию">
    <br>
    <span class="dbminputlabel">URL изображение аватара Webhook</span><br>
    <input id="webhookavatar" class="round" type="text" style="width:100%" placeholder="По желанию"><br>
    <hr class="subtlebar" style="margin-top: 4px; margin-bottom: -54px">
      ...
      <div>
      <div style="float: left; width: 35%">
      <span class="dbminputlabel">Не отправилось</span><br>
      <select id="iffalse" class="round" onchange="glob.onComparisonChanged(this)">
      <option value="0">Продолжить действия</option>
      <option value="1" selecionado>Остановить действия</option>
