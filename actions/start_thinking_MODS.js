module.exports = {
  ...
  if(data.cx == "0"){addtxt = `Сообщить пользователю: ` + (data.ephemeral ? "Приватно" : "Публично")} else {addtxt = `Добавить "Бот думает"`}
...
<option value="0" selected>Включено</option>
<option value="1">Текстовое поле (для переменных)</option>
</select>
<br>

  <div id="xinelasum">
  <dbm-checkbox id="ephemeral" label="Информировать пользователя приватно (Экесперементально)"></dbm-checkbox>
  </div>
  <div id="xinelasdois">
  <span class="dbminputlabel">Информировать пользователя приватно (используйте "true")</span><br>
