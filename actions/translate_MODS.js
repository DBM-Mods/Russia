module.exports = {
  ...
  subtitle(data) {
    if(parseInt(data.opcao, 10) == 0) {
      return `Перевести на [${data.translateTo}]`;
    }
    else {
      return `Перевести на язык участника`;
    }
  },

  variableStorage(data, varType) {
    if (parseInt(data.storage, 10) !== varType) return;
    return [data.varName, 'Строка'];
  },
...
<span class="dbminputlabel">Перевести на</span>
    <select id="opcao" class="round" onchange="glob.change(this)">
      <option value="0" selected>Аббревиатура</option>
      <option value="1">Язык участника</option>
    </select>
</div><br>
<div id="input" style="width: 200px">
<span class="dbminputlabel">Перевести на</span><br>
  <input id="translateTo" placeholder="Только 2 буквы." class="round" type="text" maxlength="2"><br>
</div>
<div>
<span class="dbminputlabel">Перевести сообщение</span><br>
  <textarea id="translateMessage" rows="6" placeholder="Введите сообщение, которое нужно перевести" style="width: 99%; font-family: monospace; white-space: nowrap; resize: none;"></textarea>
</div>
<div style="padding-top: 30px;">
  ...
  async action(cache) {
    ...
    if (!translateTo || translateTo.length > 2) return console.log('"Перевести на" может содержать только 2 буквы.');
    if (!translateMessage) return console.log('Вам нужно что-то написать для перевода.');
