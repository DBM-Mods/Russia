module.exports = {
		...
		subtitle(data, presets) {
			...
			const ordem = [`Порядок входа`,`В порядке убывания`,`Порядок возрастания`];
		return `Столбец (${[(data.dataName)]}) / Классифицировать (${ordem[parseInt(data.sort, 10)]}) / ${storage[parseInt(data.storage, 10)]} (${data.varName2})`
	},
	...
	html: function (isEvent, data) {
		...
		<span class="dbminputlabel">Расположение файла данных</span><br>
		<input id="alvo" value="./data/players" class="round" type="text">
<br>
		<span class="dbminputlabel">Название столбца</span><br>
		<input id="dataName" class="round" type="text">
<br>

<table>
  		<tr>
 		<td class="sep3"><span class="dbminputlabel">Классифицировать</span><br>
		<select id="sort" class="round" style="width: 100%;">
		<option value="0" selected>Порядок входа</option>
		<option value="1" selected>В порядке убывания</option>
		<option value="2">В порядок возрастания</option>
	   	</select></td>
		<td class="sep4">
		<span class="dbminputlabel">Начиная с</span><br>
		<input id="inicio" class="round" type="text" placeholder="Альтернативный">
		</td>
		<td class="sep4">
		<span class="dbminputlabel">До</span><br>
		<input id="maxresult" class="round" type="text" placeholder="Альтернативный">
		</td>
  		<td class="sep5"><br><dbm-checkbox id="debug" label="Режим отладки" checked></dbm-checkbox></td>
 		</tr>
  		</table>		
<br>
...
<br>
		<table>
 		...
 		<select id="storage" class="round" onchange="glob.variableChange(this, 'varNameContainer2')">
  		...
  		<td class="sep2"><div id="varNameContainer2"><span class="dbminputlabel">Имя переменной</span><br>
  ...
  `
	...
	async action (cache) {
		...
		if (file) {
			...
			if(debug == true){
					console.log(`Организация столбца (${this.evalMessage(data.dataName, cache)})`)
				}
			...
			if (val !== undefined) {
				...
				try {
					...
					if(debug == true){
						if(retornar == 3){
							console.log(`Поиск рейтинга (${rankid}) в ${result.length} упорядоченные столбцы`)
						} else {
					console.log(`Сбор из ${inicio} а ${maxresult} в ${result.length} упорядоченные столбцы`)}
				}
				...
				console.log("Data Ranking MOD: столбец не существует!")}
				}
