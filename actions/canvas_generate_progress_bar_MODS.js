module.exports = {
  ...
  subtitle(data) {
    ...
    if (parseInt(data.type) >= 100) {
      var type2 = ['Брови', 'Звезда']
      var tipobar = type2[index - 100]
    } else {
      var type = ['Прямой', 'Круг', 'Дуга', 'Изображение']
    ...
    return data.description
      ...
      : `<font style="color:${desccor}">Прогресс-бар: ${tipobar} - ${storeTypes[parseInt(data.storage)]} (${data.varName})</font>`
  },
...
<span class="dbminputlabel">Тип бара</span><br>
<select id="type" class="round" onchange="glob.onChange1(this)">
  <option value="0" selected>Прямой</option>
  <option value="1">Круг</option>
  <option value="2">Дуга</option>
  <option value="3">Изображение</option>
  <option value="100">Бровь</option>
  <option value="101">Звезда</option>
</select>
</td>
...
<select id="lineCap" class="round" onchange="glob.onChange1x(this)">
  <option value="0" selected>Квадрат</option>
...
<br>
  ...
  <span class="dbminputlabel">Ширина изображения</span><br>
    <input id="iwidth" class="round" type="text" placeholder="Оставить пустым для значения по умолчанию" value="100%"><br>
  </div>
  <div style="float: left; width: 34%;padding:0px 5px 0px 0px">
  <span class="dbminputlabel">Высота изображения</span><br>
    <input id="iheight" class="round" type="text" placeholder="Оставить пустым для значения по умолчанию" value="100%"><br>
  </div>
  <div style="float: right; width: 33%">
  <span class="dbminputlabel">Вер-ное положение</span><br>
    <input id="ipos" class="round" type="text" placeholder="Оставить пустым для значения по умолчанию" value="0"><br>
  </div>
...
<span class="dbminputlabel"><span id="Change1text">Ширина бара</span></span><br>
<input id="width" class="round" type="text">
</td>
<td style="width:33% !important" id="quadratico">
<span class="dbminputlabel"><span id="Change2text">Высота бара</span></span><br>
<input id="height" class="round" type="text">
</td>
<td style="width:33% !important" id="circulatico">
<span class="dbminputlabel">Высота линии</span><br>
<input id="lineWidth" class="round" type="text">
</td>
</tr></table>

<div style="padding: 8px 5px 0px 5px">
<span class="dbminputlabel">Заполненость в %</span><br>
...
<tab label="Цвет фона" icon="align left">
...
<select id="tipocor2" class="round" onchange="glob.onChange2(this)">
  ...
  <option value="2">Цвет градиента / EVAL</option>
  <option value="3">Цвет градиента / Выбор</option>
</select> </div>
...
<span class="dbminputlabel">Направление</span>
  <select id="apartir2" class="round">
    <option value="0">Сверху вниз</option>
    <option value="1">Левая диагональ сверху вниз</option>
    <option value="2" selected>Справа налево</option>
    <option value="3">Правая диагональ сверху вниз</option>
    <option value="4">Снизу вверх</option>
    <option value="5">Левая диагональ снизу вверх</option>
    <option value="6">Слева направо</option>
    <option value="7">Правая диагональ снизу вверх</option>
    <option value="8">Радиально</option>
  </select>
</div>
</div>
...
<dialog-list id="gradient2" fields='["posicao", "cor"]' dialogTitle="Добавить цвет" dialogResizable dialogWidth="400" dialogHeight="220" listLabel="Градиент" listStyle="height: calc(100vh - 330px);" itemName="Embed" itemCols="1" itemHeight="30px;" itemTextFunction="glob.formatItem2(data)" itemStyle="text-align: left; line-height: 30px;">
...
<span class="dbminputlabel">Цвет фона (HEX или RGBA)</span><br>
    <table style="width:100%"><tr><th><input id="colorfundo" name="actionxinxyla" class="round" type="text" placeholder="Дополнительно" onchange="(function(){
...
<dialog-list id="gradient3" fields='["posicao", "cor"]' dialogTitle="Настроить цвет" dialogResizable dialogWidth="400" dialogHeight="220" listLabel="Градиент" listStyle="height: calc(100vh - 330px);" itemName="Embed" itemCols="1" itemHeight="30px;" itemTextFunction="glob.formatItem(data)" itemStyle="text-align: left; line-height: 30px;">
<div style="padding: 12px">
...
<span class="dbminputlabel">Цвет (HEX или RGBA)</span><br>
    <table style="width:100%"><tr><th><input id="color" name="actionxinxyla" class="round" type="text" placeholder="Требуется" onchange="(function(){
    ...
    <th style="width:60px;text-align:center;padding:4px"><a id="btr1" style="cursor:pointer" onclick="(function(){
      ...
      })()"><button class="tiny compact ui icon button">Выбор</button><div style="float:right"><div style="min-width:15px;margin-top:4px" id="exib2">&#8203;</div></div></a><a id="btr2" style="cursor:pointer;display:none" onclick="(function(){
...
<tab label="Оттенок" icon="align left">
<div style="padding:0px 4px;padding-top:8px;height: calc(100vh - 200px);overflow:auto">
...
<table style="width:100%"><tr>
<td style="width:50% !important">
<span class="dbminputlabel">Размер тени</span><br>
<input id="blur2" class="round" value="0" type="text">
</td>
<td style="width:50% !important">
<span class="dbminputlabel">Интенсивность</span><br>
<input id="int2" class="round" value="1" type="text">
</td>
</tr></table>

    <br>
    <span class="dbminputlabel">Цвет оттенка</span><br>
    <table style="width:100%"><tr><th><input id="shadowcor2" name="actionxinxyla" class="round" type="text" placeholder="Дополнительно" onchange="(function(){
...
Если функция недоступна, то следующая операция с холстом не позволяет создать фоновую тень, так как закругленный край обрезается
</div>
...
<td style="width:50% !important">
<span class="dbminputlabel">Интенсивностьe</span><br>
<input id="int" class="round" value="1" type="text">
   ...
   <br>
    ...
    <span class="dbminputlabel">Цвет оттенка</span><br>
    <table style="width:100%"><tr><th><input id="shadowcor" name="actionxinxyla" class="round" type="text" placeholder="Дополнительно" onchange="(function(){
    ...
    <th style="width:60px;text-align:center;padding:4px"><a id="3btr1" style="cursor:pointer" onclick="(function(){
      ...
      document.getElementById('3btr2').style.display = 'block';
      })()"><button class="tiny compact ui icon button">Выбор</button><div style="float:right"><div style="min-width:15px;margin-top:4px" id="exib4">&#8203;</div></div></a><a id="3btr2" style="cursor:pointer;display:none" onclick="(function(){
       ...
       ⚠️ Частично недоступная функция, тень будет находиться внутри цвета фона
</div>
...
xinspace{padding:5px 0px 0px 0px;display:block}
  ...
  init() {
    ...
    glob.onChange1 = function (event) {
      ...
      if (event.value === '0') {
        Change1text.innerHTML = 'Ширина бара'
        Change2text.innerHTML = 'Высота бара'
      ...
      if (event.value === '3') {
        ...
        Change1text.innerHTML = 'Радиус (2x высота/ширина)'
    ...
    glob.formatItem2 = function (data) {
      ...
      result2 += `<div style="float:left;width:50%;overflow:hidden;padding-left:10px;"> Позиция: ${data.posicao} </div>`
      result2 += `<div style="float:right;width:50%;overflow:hidden;"> Цвет: ${data.cor} </div>`
      return result2;
    }
