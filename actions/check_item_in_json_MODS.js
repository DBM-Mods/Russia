module.exports = {
  ...
  subtitle: function (data, presets) {
    ...
    const opcao = [
      ...
      "Это URL изображения?",
      "Это URL?",
      `Включает ~ Игнорировать регистр "${data.value}"`,
      `Включает ~ Игнорировать ударения "${data.value}"`,
      `Включает ~ Игнорировать ударения и регистр "${data.value}"`,
    ]
...
<div style="width: 100%">
  <span class="dbminputlabel">Сравнение на основе столбцов (используйте . для подстолбцов)</span><br>
...
xinspace{padding:5px 0px 0px 0px;display:block}
  ...
  init() {
    ...
    glob.onComparisonChanged2 = function (event) {
      ...
      if (event.value == "4") {
        document.querySelector("[id='xinelas2']").innerText = (`Имя якоря`);
      }
    }
