...
if (data.retornar == 1) {
  return data.description
  ? `<font style="color:${desccor}">${data.description}</font>`
  : `<font style="color:${desccor}">Просмотр объекта из ${storage[parseInt(data.storage, 10)]} (${data.varName})</font>`
}
...
<td style="padding:0px 0px 0px 10px;width:70px"><div style="float:left;padding:0px 0px 0px 7px;margin-top:-5px"><dbm-checkbox id="descriptionx" label="Цвет (вкл)"></dbm-checkbox></div><br><input type="color" value="#ffffff" class="round" id="descriptioncolor"></td>
    ...
    <div style="width:100%;">
        <span class="dbminputlabel">Вернуть</span>
        <select id="retornar" class="round" onchange="glob.onChange1(this)">
            <option value="0" selected>Текст с цветом в консоли</option>
            <option value="2">Текст с цветом в консоли через EVAL</option>
            <option value="1">Показать информацию об объекте переменной</option>
        </select>
    </div>
  <br>
  ...
  <span class="dbminputlabel">Переменная с объектом</span><br>
...
<span class="dbminputlabel">Цвет фона</span><br>
<table style="width:100%"><tr><td><input id="bg" name="actionxinxyla" class="round" type="text" placeholder="Не обязательное поле"><td>
<td style="width:40px;text-align:center;padding:4px"><a id="2btr1" style="cursor:pointer" onclick="(function(){
  ...
  document.getElementById('2btr2').style.display = 'block';
    ...
    <span class="dbminputlabel">Цвет текста</span><br>
...
<div>
<span class="dbminputlabel">Сообщение для отправки</span><br>
  ...
  <center>Пример<br>
  chalk.hex('#c01111')('текст') + chalk.hex('#1cc819')('текст2') + chalk.bgHex('#1cc819')('текст3') + chalk.hex('#111111').bgHex('#1cc819')('текст4')</center>
  </div>
</div>
</div>
...
`
  ...
  action (cache) {
    ...
    if (send.length > 0) {
      ...
      console.log(`Предоставьте что-нибудь для записи: Действие #${cache.index + 1}`)
      this.callNextAction(cache)
    }}
